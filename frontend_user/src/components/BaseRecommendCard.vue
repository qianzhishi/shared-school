<script lang="ts" setup>
    ...
    const postInfo = defineProps({
        info: {
            type: Object as () => recommendCardInfo,
            required: true,
        },
    })
    ...
    const cardType = ref<number>();
    const userId = ref<number>();
    const userAvatar = ref<string>();
    const userName = ref<string>();
    const postTitle = ref<string>();
    const postContent = ref<string>("");
    const readCount = ref<number>();
    ...
    // 对过长的用户名进行截取
    function nameTranslate(name: string) {
        if (name.length > 20) {
    ...
    // 前往详情页面
    function openDetail() {
        router.push({ path: '/detail', query: { id: cardId.value, type: cardType.value } });
    }
...
<template>
    <el-card class="card"
        :class="{ 'study-card': cardType === 1, 'play-card': cardType === 2, 'love-card': cardType === 3}"
        shadow="hover" @click="openDetail()">
        <div class="header">
            <div class="user-info">
                <div @click.stop="router.push({ path: '/user', query: { id: userId } })">
                    <el-avatar class="avatar" :src="userAvatar" v-if="userAvatar != ''" />
        ...
        <div class="content">
            <div class="post-detail-area">
                <h1 class="post-title-detail">{{ postTitle }}</h1>
                <div class="post-content-detail" v-html="postContent"></div>
            </div>
            <div class="post-images-area">
                <el-carousel height="35vh" direction="horizontal" type="card" :autoplay="false"
                    :indicator-position="'none'">
                    <el-carousel-item v-for="(item, index) in postImages" :key="index">
                        <el-image style="width: 35vh; height: 35vh;border-radius: 2vh;" :src="item" :fit="'cover'">
                            <template #error>
        ...
        <div class="footer">
            <div class="info-bar">
                ...
                <div class="info-element">
                    <svg t="1715593642941" class="comment-icon" viewBox="0 0 1024 1024" version="1.1"
